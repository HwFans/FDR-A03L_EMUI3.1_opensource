...
#include <huawei_platform/sensor/bstclass.h>
...
static int bma2x2_debug_mask= 1;

module_param_named(bma2x2_debug, bma2x2_debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP);

#define BMA2X2_ERR(x...) do {\
	if (bma2x2_debug_mask >=0) \
        printk(KERN_ERR x);\
    } while (0)
#define BMA2X2_WARN(x...) do {\
	if (bma2x2_debug_mask >=0) \
        printk(KERN_ERR x);\
    } while (0)
#define BMA2X2_INFO(x...) do {\
	if (bma2x2_debug_mask >=1) \
        printk(KERN_ERR x);\
    } while (0)
#define BMA2X2_DEBUG(x...) do {\
    if (bma2x2_debug_mask >=2) \
        printk(KERN_ERR x);\
    } while (0)
...
#define BMAX2X2_OFFSET_DATA_X	0x0
#define BMAX2X2_OFFSET_DATA_Y	0x0
#define BMAX2X2_OFFSET_DATA_Z	0x0
		...
		u8 *data, u16 len)
{
	...
	if (BMA_MAX_RETRY_I2C_XFER <= retry) {
		BMA2X2_ERR("%s line:%d I2C xfer error \n",__func__,__LINE__);
		return -EIO;
	}
					...
					struct bma2x2_data *data)
{
	...
	while (read_count++ < CHECK_CHIP_ID_TIME_MAX) {
		err = bma2x2_smbus_read_byte(client, BMA2X2_CHIP_ID_REG,
							&chip_id);
		if (err < 0) {
			BMA2X2_ERR("%s line:%d Bosch Sensortec Device not found"
		...
		for (i = 0; i < bma2x2_sensor_type_count; i++) {
			if (sensor_type_map[i].chip_id == chip_id) {
				...
				BMA2X2_ERR("%s line:%d Bosch Sensortec Device detected, "
					"HW IC name: %s\n",__func__,__LINE__,sensor_type_map[i].sensor_name);
				dev_dbg(&client->dev,
									"Bosch Sensortec Device detected,"
									" HW IC name: %s\n",
									sensor_type_map[i].sensor_name);
				/* < DTS2015120303965  chendong 20151126 begin */
				err = app_info_set("G-Sensor",  sensor_type_map[i].sensor_name);	
				if (err < 0)/*failed to add app_info*/
      				{
        			        BMA2X2_ERR("%s %d:failed to add app_info\n", __func__, __LINE__);
			        }
				/* DTS2015120303965  chendong 20151126 end > */
				data->chip_type = i;
				return err;
		...
		if (i < bma2x2_sensor_type_count) {
			...
			BMA2X2_ERR("%s line:%d Failed!Bosch Sensortec Device"
...
static int bma2x2_set_mode(struct i2c_client *client, unsigned char mode)
{
	...
	if (mode < 6) {
		...
		case BMA2X2_MODE_SUSPEND:
			...
			if (ret < 0)
				BMA2X2_ERR("%s line:%d Error switching to suspend \n",__func__,__LINE__);
			break;
...
static int bma2x2_sel_int1_pad(const struct bma2x2_data *data)
{
	...
	if (err) {
		BMA2X2_ERR("%s line:%d select pad int1 error, ret=%d\n",__func__,__LINE__,err);
		err = -EIO;
	}
	return err;
}
		...
		const char *buf, size_t count)
{
...
#ifdef CONFIG_SENSORS_BMI058
	BMA2X2_INFO("%s line:%d diff y is %d,value1 is %d, value2 is %d \n",
		__func__,__LINE__,diff,value1, value2);
...
#ifdef CONFIG_SENSORS_BMI058
	BMA2X2_INFO("%s line:%d diff x is %d,value1 is %d, value2 is %d \n",
		__func__,__LINE__,diff,value1, value2);
	test_result_branch = 1;
#else
	BMA2X2_INFO("%s line:%d diff y is %d,value1 is %d, value2 is %d \n",
	...
	bma2x2_read_accel_z(bma2x2->bma2x2_client,
					bma2x2->sensor_type, &value2);
	diff = value1-value2;
	BMA2X2_INFO("%s line:%d diff z is %d,value1 is %d, value2 is %d \n",
		__func__,__LINE__,diff,value1, value2);
	...
	bma2x2_soft_reset(bma2x2->bma2x2_client);
	RESET_DELAY();
	BMA2X2_WARN("%s line:%d self test finished\n",__func__,__LINE__);
	return count;
}
			...
			struct bma2x2acc *value)
{
	...
	if (err < 0) {
		BMA2X2_WARN("%s line:%d read accel data failed! err = %d\n",__func__,__LINE__,err);
		return;
	}
	/* < DTS2015120303965  chendong 20151126 begin */
	value->x =value->x >> 4;
		...
		const char *buf, size_t count)
{
	...
	if (data > 7 && data < 0) {
		BMA2X2_ERR("%s line:%d Invalid value\n",__func__,__LINE__);
		return count;
	} else {
		bma2x2->pdata->place = data;
		BMA2X2_ERR("%s line:%d  bma2x2 place is set to %d\n",__func__,__LINE__,bma2x2->pdata->place);
...
static int bma2x2_config_interrupt(struct bma2x2_data *data, int enable)
{
	...
	if (data->pdata->use_int2) {
		...
		if (err) {
			BMA2X2_ERR("%s line:%d  Failed to select int2 pad, err=%d\n",__func__,__LINE__,err);
			goto exit;
		}

		err = bma2x2_set_int2_active_lvl(client, act_high);
		if (err) {
			BMA2X2_ERR("%s line:%d  Failed to select int2 level, err=%d\n",__func__,__LINE__,err);
			goto exit;
		}
	} else {
		err = bma2x2_sel_int1_pad(data);
		if (err) {
			BMA2X2_ERR("%s line:%d  Failed to select int1 pad, err=%d\n",__func__,__LINE__,err);
			goto exit;
		}
	...
	if (err) {
		BMA2X2_ERR("%s line:%d  Failed to set interrupt latch, err=%d\n",__func__,__LINE__,err);
		goto exit;
	}
...
static void bma2x2_set_enable(struct device *dev, int enable)
{
	...
	if (atomic_read(&bma2x2->cal_status)) {
		BMA2X2_ERR("%s line:%d  can not enable or disable when calibration\n",__func__,__LINE__);
		return;
	}
	...
	if (enable) {
		if (pre_enable == 0) {
			if (bma2x2_power_ctl(bma2x2, true)) {
				BMA2X2_ERR("%s line:%d  power failed\n",__func__,__LINE__);
				goto mutex_exit;
			}
			if (bma2x2_open_init(client, bma2x2) < 0) {
				BMA2X2_ERR("%s line:%d  set init failed\n",__func__,__LINE__);
				goto mutex_exit;
			}
			...
			if ((bma2x2->pdata->int_en) &&
				...
				if (bma2x2_config_interrupt(bma2x2, true)) {
					BMA2X2_ERR("%s line:%d config interrupt failed\n",__func__,__LINE__);
					goto mutex_exit;
				}
				if (bma2x2_set_Int_Enable(client,
					BMA2X2_DATA_EN, 1)) {
					BMA2X2_ERR("%s line:%d enable interrupt failed\n",__func__,__LINE__);
					goto mutex_exit;
				}
		...
		if (pre_enable == 1) {
			if (bma2x2_store_state(client, bma2x2) < 0) {
				BMA2X2_ERR("%s line:%d set state failed\n",__func__,__LINE__);
				goto mutex_exit;
			}
			bma2x2_set_mode(bma2x2->bma2x2_client,
					BMA2X2_MODE_SUSPEND);
			...
			if ((bma2x2->pdata->int_en) &&
				...
				if (bma2x2_set_Int_Enable(client,
					BMA2X2_DATA_EN, 0)) {
					BMA2X2_ERR("%s line:%d disable interrupt failed\n",__func__,__LINE__);
					goto mutex_exit;
				}

				if (bma2x2_config_interrupt(bma2x2, false)) {
					BMA2X2_ERR("%s line:%d deconfig interrupt failed\n",__func__,__LINE__);
					goto mutex_exit;
				}
			} else {
				#if 0
				cancel_delayed_work_sync(&bma2x2->delay_work);
				#else
				hrtimer_cancel(&bma2x2->poll_timer);
				cancel_work_sync(&bma2x2->delay_work.work);
				#endif
			}
...
mutex_exit:
	...
	BMA2X2_ERR("%s line:%d set enable: en=%d, en_state=%d, use_int=%d\n",
		__func__,__LINE__,enable, atomic_read(&bma2x2->enable),bma2x2->pdata->int_en);
...
static int bma2x2_select_chanel(struct i2c_client *client)
{
	...
	if (bma2x2->pdata->place > 3 && bma2x2->pdata->place < 8)
		...
		BMA2X2_ERR("%s line:%d unknown sensor place\n",__func__,__LINE__);

		return -EINVAL;
	}
	if (bma2x2_set_mode(client, BMA2X2_MODE_NORMAL) < 0) {
		BMA2X2_ERR("%s line:%d set calibrate mode error\n",__func__,__LINE__);
		return -EINVAL;
	}
	if (bma2x2_set_bandwidth(client, BMA2X2_BW_1000HZ) < 0) {
		BMA2X2_ERR("%s line:%d set calibrate bandwidth error\n",__func__,__LINE__);
		return -EINVAL;
	}
	if (bma2x2_set_range(client, BMA2X2_RANGE_SET) < 0) {
		BMA2X2_ERR("%s line:%d set calibrate range error\n",__func__,__LINE__);
	...
	for (i = 0; i < 3; i++) {
		if (bma2x2_set_offset_target(client, bmi058_channel_tb[i],
			...
			BMA2X2_ERR("%s line:%d set offset target error\n",__func__,__LINE__);
			return -EINVAL;
		}
		if (bma2x2_set_cal_trigger(client, (i + 1)) < 0) {
			BMA2X2_ERR("%s line:%d read calibration state error\n",__func__,__LINE__);
			return -EINVAL;
		}
		timeout = 0;
		do {
			...
			if (error < 0) {
				BMA2X2_ERR("%s line:%d read cal_ready error\n",__func__,__LINE__);
				return error;
			}
			timeout++;
			if (timeout == RETRY_TIME) {
				BMA2X2_ERR("%s line:%d get fast calibration ready error\n",__func__,__LINE__);
				return -EINVAL;
			};
	...
	if (error < 0) {
		BMA2X2_ERR("%s line:%d restore calibrate bandwidth error\n",__func__,__LINE__);
		return error;
	}
	return 0;
}
		...
		int axis, int apply_now)
{
	...
	if (atomic_cmpxchg(&data->cal_status, 0, 1)) {
		BMA2X2_ERR("%s line:%d do calibration error \n",__func__,__LINE__);
		return -EBUSY;
	}

	error = bma2x2_power_ctl(data, true);
	if (error) {
		BMA2X2_ERR("%s line:%d Failed to enable sensor power \n",__func__,__LINE__);
		error = -EINVAL;
		goto exit;
	}
	error = bma2x2_select_chanel(client);
	if (error < 0) {
		BMA2X2_ERR("%s line:%d xyz calibration error \n",__func__,__LINE__);
		goto exit;
	}
	BMA2X2_ERR("%s line:%d xyz axis fast calibration finished \n",__func__,__LINE__);
	error = bma2x2_eeprom_prog(client);
	if (error < 0) {
		BMA2X2_ERR("%s line:%d wirte calibration to eeprom failed \n",__func__,__LINE__);
		goto exit;
	}
	...
	if (error) {
		BMA2X2_ERR("%s line:%d Failed to disable sensor power \n",__func__,__LINE__);
		goto exit;
	}
...
static int bma2x2_eeprom_prog(struct i2c_client *client)
{
	...
	res = bma2x2_smbus_read_byte(client, BMA2X2_EEPROM_CTRL_REG,
					&databuf);
	if (res < 0) {
		BMA2X2_ERR("%s line:%d read eeprom control reg error1 \n",__func__,__LINE__);
		return res;
	}
	databuf |= 0x01;
	res = bma2x2_smbus_write_byte(client, BMA2X2_EEPROM_CTRL_REG,
					&databuf);
	if (res < 0) {
		BMA2X2_ERR("%s line:%d write eeprom control reg error1 \n",__func__,__LINE__);
		return res;
	}
	...
	res = bma2x2_smbus_read_byte(client, BMA2X2_EEPROM_CTRL_REG,
					&databuf);
	if (res < 0) {
		BMA2X2_ERR("%s line:%d read eeprom control reg error2 \n",__func__,__LINE__);
		return res;
	}
	databuf |= 0x02;
	res = bma2x2_smbus_write_byte(client, BMA2X2_EEPROM_CTRL_REG,
					&databuf);
	if (res < 0) {
		BMA2X2_ERR("%s line:%d write eeprom control reg error2\n",__func__,__LINE__);
		return res;
	}
	do {
		WAIT_CAL_READY();
		res = bma2x2_smbus_read_byte(client, BMA2X2_EEPROM_CTRL_REG,
					&databuf);
		if (res < 0) {
			BMA2X2_ERR("%s line:%d read nvm_rdy error\n",__func__,__LINE__);
			return res;
		}
		databuf = (databuf >> 2) & 0x01;
		if (++timeout == 50) {
			BMA2X2_ERR("%s line:%d check nvm_rdy time out\n",__func__,__LINE__);
			break;
		}
	} while (databuf == 0);
	...
	res = bma2x2_smbus_read_byte(client, BMA2X2_EEPROM_CTRL_REG,
					&databuf);
	if (res < 0) {
		BMA2X2_ERR("%s line:%d read eeprom control reg error3\n",__func__,__LINE__);
		return res;
	}
	databuf &= 0xFE;
	res = bma2x2_smbus_write_byte(client, BMA2X2_EEPROM_CTRL_REG,
					&databuf);
	if (res < 0) {
		BMA2X2_ERR("%s line:%d write eeprom control reg error3\n",__func__,__LINE__);
		return res;
	}
	return res;
}
		...
		const char *buf, size_t count)
{
	...
	BMA2X2_ERR("%s line:%d x axis fast calibration finished\n",__func__,__LINE__);
		...
		const char *buf, size_t count)
{
	...
	BMA2X2_ERR("%s line:%d y axis fast calibration finished\n",__func__,__LINE__);
	return count;
}
		...
		const char *buf, size_t count)
{
	...
	BMA2X2_ERR("%s line:%d z axis fast calibration finished\n",__func__,__LINE__);
...
static void bma2x2_tap_timeout_handle(unsigned long data)
{
	...
	BMA2X2_ERR("%s line:%d tap interrupt handle, timeout\n",__func__,__LINE__);
...
static void bma2x2_high_g_interrupt_handle(struct bma2x2_data *bma2x2)
{
	...
	for (i = 0; i < 3; i++) {
		...
		BMA2X2_ERR("%s line:%d High G interrupt happened,exis is %d,"
			"first is %d,sign is %d\n", __func__,__LINE__,i,first_value, sign_value);
...
static void bma2x2_slope_interrupt_handle(struct bma2x2_data *bma2x2)
{
	...
	for (i = 0; i < 3; i++) {
		...
		BMA2X2_ERR("%s line:%d Slop interrupt happened,exis is %d,"
			"first is %d,sign is %d\n", __func__,__LINE__, i,first_value, sign_value);
	}
}
#endif
...
static int bma2x2_data_ready_handle(struct bma2x2_data *bma2x2)
{
	...
	if (ret) {
		BMA2X2_ERR("%s line:%d read interrupt status2 err, err=%d\n", __func__,__LINE__, ret);
		return -EIO;
	}
	...
	if (status != 0) {
		BMA2X2_ERR("%s line:%d Interrupt flag is detected, state2 =0x%x\n", __func__,__LINE__, status);
		return -EAGAIN;
	}
	...
	ret = bma2x2_get_interruptstatus1(bma2x2->bma2x2_client,
			&status);
	if (ret) {
		BMA2X2_ERR("%s line:%d read interrupt status1 err, err=%d\n", __func__,__LINE__, status);
		return -EIO;
	}
	...
	BMA2X2_ERR("%s line:%d Data ready int is not detected, state1 =0x%x\n", __func__,__LINE__, status);
...
static void bma2x2_irq_work_func(struct work_struct *work)
{
	...
	/* < CQ_hw00003916 yanfei 20151121 begin */
	BMA2X2_ERR("%s line:%d bma2x2_irq_work_func, status=%d\n", __func__,__LINE__, status);

#ifdef CONFIG_SIG_MOTION
	if (status & 0x04)	{
		if (atomic_read(&bma2x2->en_sig_motion) == 1) {
			BMA2X2_ERR("%s line:%d Significant motion interrupt happened\n", __func__,__LINE__);
			/* close sig sensor,
	...
	if (status & 0x20) {
		if (atomic_read(&bma2x2->en_double_tap) == 1) {
			BMA2X2_ERR("%s line:%d single tap interrupt happened\n", __func__,__LINE__);
			...
			if (bma2x2->tap_times == 0)	{
				...
				BMA2X2_ERR("%s line:%d double tap\n", __func__,__LINE__);
				mutex_lock(&bma2x2->tap_mutex);
				bma2x2->tap_times = 0;
	...
	case 0x01:
		BMA2X2_ERR("%s line:%d Low G interrupt happened\n", __func__,__LINE__);
	...
	case 0x08:
	/* CQ_hw00003916 yanfei 20151121 end > */
		BMA2X2_ERR("%s line:%d slow/ no motion interrupt happened\n", __func__,__LINE__);
	...
	case 0x10:
		BMA2X2_ERR("%s line:%d double tap interrupt happened\n", __func__,__LINE__);
		input_report_rel(bma2x2->dev_interrupt,
			DOUBLE_TAP_INTERRUPT,
			DOUBLE_TAP_INTERRUPT_HAPPENED);
		break;
	...
	case 0x40:
		bma2x2_get_orient_status(bma2x2->bma2x2_client,
				    &first_value);
		BMA2X2_ERR("%s line:%d orient interrupt happened,%s\n", __func__,__LINE__,orient[first_value]);
		if (first_value == 0)
	...
	case 0x80:
		bma2x2_get_orient_flat_status(bma2x2->bma2x2_client,
				    &sign_value);
		BMA2X2_ERR("%s line:%d flat interrupt happened,flat status is %d\n", __func__,__LINE__,sign_value);
		if (sign_value == 1) {
			input_report_abs(bma2x2->dev_interrupt,
				FLAT_INTERRUPT,
...
static void bma2x2_irq_work_func(struct work_struct *work)
{
	BMA2X2_ERR("%s line:%d Interrupt feature is not enabled!\n", __func__,__LINE__);
	return;
}
...
static int bma2x2_power_ctl(struct bma2x2_data *data, bool on)
{
	...
	if (!on && data->power_enabled) {
		...
		if (ret) {
			...
			BMA2X2_ERR("%s line:%d Regulator vdd disable failed ret=%d\n", __func__,__LINE__,ret);
		...
		if (ret) {
			BMA2X2_ERR("%s line:%d Regulator vio disable failed ret=%d\n", __func__,__LINE__,ret);
		...
		if (ret) {
			BMA2X2_ERR("%s line:%d Regulator vdd enable failed ret=%d\n", __func__,__LINE__,ret);
			return ret;
		}

		ret = regulator_enable(data->vio);
		if (ret) {
			BMA2X2_ERR("%s line:%d Regulator vio enable failed ret=%d\n", __func__,__LINE__,ret);
			err = regulator_disable(data->vdd);
			return ret;
		}
		data->power_enabled = on;
	} else {
		BMA2X2_ERR("%s line:%d Power on=%d. enabled=%d\n", __func__,__LINE__,on, data->power_enabled);
	}
...
static int bma2x2_power_init(struct bma2x2_data *data)
{
	...
	if (IS_ERR(data->vdd)) {
		...
		BMA2X2_ERR("%s line:%d Regulator get failed vdd ret=%d\n", __func__,__LINE__,ret);
		return ret;
	}
	...
	if (regulator_count_voltages(data->vdd) > 0) {
		ret = regulator_set_voltage(data->vdd,
				...
				BMA2x2_VDD_MAX_UV);
		if (ret) {
			BMA2X2_ERR("%s line:%d Regulator set failed vdd ret=%d\n", __func__,__LINE__,ret);
			goto reg_vdd_put;
		}
	}
	...
	if (IS_ERR(data->vio)) {
		...
		BMA2X2_ERR("%s line:%d Regulator get failed vio ret=%d\n", __func__,__LINE__,ret);
		goto reg_vdd_set;
	}
	...
	if (regulator_count_voltages(data->vio) > 0) {
		ret = regulator_set_voltage(data->vio,
				...
				BMA2x2_VIO_MAX_UV);
		if (ret) {
			BMA2X2_ERR("%s line:%d Regulator set failed vio ret=%d\n", __func__,__LINE__,ret);
			goto reg_vio_put;
		}
	}
			...
			struct bma2x2_platform_data *pdata)
{
	...
	if (rc && (rc != -EINVAL)) {
		BMA2X2_ERR("%s line:%d Unable to read init-interval\n", __func__,__LINE__);
	...
	if (rc && (rc != -EINVAL)) {
		BMA2X2_ERR("%s line:%d Unable to read sensor place paramater\n", __func__,__LINE__);
		return rc;
	}
	if (temp_val > 7 || temp_val < 0) {
		BMA2X2_ERR("%s line:%d Invalid place parameter, use default value 0\n", __func__,__LINE__);
		pdata->place = 0;
	} else {
			...
			struct bma2x2_data *data)
{
	...
	if (err < 0) {
		BMA2X2_ERR("%s line:%d init bandwidth error\n", __func__,__LINE__);
		return err;
	}
	...
	bma2x2_set_offset_x(client,BMAX2X2_OFFSET_DATA_X);
	bma2x2_set_offset_y(client,BMAX2X2_OFFSET_DATA_Y);
	bma2x2_set_offset_z(client,BMAX2X2_OFFSET_DATA_Z);
			...
			const unsigned int gpio)
{
	...
	if (!gpio_is_valid(gpio)) {
		BMA2X2_ERR("%s line:%d gpio(%d) is invalid\n", __func__,__LINE__,gpio);
		return -EINVAL;
	}

	err = gpio_request(gpio, "bma2x2_gpio_int");
	if (err) {
		BMA2X2_ERR("%s line:%d Unable to request gpio %d, err=%d\n", __func__,__LINE__,gpio, err);
		return err;
	}

	err = gpio_direction_input(gpio);
	if (err) {
		BMA2X2_ERR("%s line:%d Unable to set gpio direction %d, err=%d\n", __func__,__LINE__,gpio, err);
		gpio_free(gpio);
		return err;
	}

	client->irq = gpio_to_irq(gpio);
	BMA2X2_ERR("%s line:%d Interrupt gpio=%d, irq=%d\n", __func__,__LINE__,gpio, client->irq);
	return 0;
}
...
static int bma2x2_pinctrl_init(struct bma2x2_data *data)
{
	...
	if (IS_ERR_OR_NULL(pctrl)) {
		ret = PTR_ERR(pctrl);
		BMA2X2_ERR("%s line:%d Failed to get pin pinctrl, err:%d\n", __func__,__LINE__,ret);
		goto exit;
	}
	pctrl_data = devm_kzalloc(&client->dev,
			sizeof(*pctrl_data), GFP_KERNEL);
	if (!pctrl_data) {
		BMA2X2_ERR("%s line:%d No memory for pinctrl data\n", __func__,__LINE__);
		ret = -ENOMEM;
		goto exit;
	}
	...
	if (IS_ERR_OR_NULL(pctrl_data->pins_default)) {
		...
		BMA2X2_ERR("%s line:%d Could not get default pinstate, err:%d\n", __func__,__LINE__,ret);
		goto exit;
	}
	...
	if (IS_ERR_OR_NULL(pctrl_data->pins_sleep)) {
		BMA2X2_ERR("%s line:%d Could not get sleep pinstate, err:%ld\n", __func__,__LINE__,PTR_ERR(pctrl_data->pins_sleep));
			...
			bool active)
{
	...
	if (active) {
		...
		if (ret)
			BMA2X2_ERR("%s line:%d Select default pinstate err:%d\n", __func__,__LINE__,ret);

	} else {
		if (!data->pctrl_data->pins_sleep) {
			BMA2X2_ERR("%s line:%d Pinstate 'sleep' is not defined\n", __func__,__LINE__);
		} else {
			ret = pinctrl_select_state(data->pctrl_data->pctrl,
					data->pctrl_data->pins_sleep);
			if (ret)
				BMA2X2_ERR("%s line:%d Select sleep pinstate err:%d\n", __func__,__LINE__,ret);
		}
	}
	BMA2X2_ERR("%s line:%d Select pinctrl state=%d\n", __func__,__LINE__,active);
		...
		const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		BMA2X2_ERR("%s line:%d i2c_check_functionality error\n", __func__,__LINE__);
		err = -EPERM;
		goto exit;
	}
	...
	if (client->dev.of_node) {
		...
		if (!pdata) {
			BMA2X2_ERR("%s line:%d Failed to allcated memory\n", __func__,__LINE__);
			err = -ENOMEM;
			goto kfree_exit;
		}
		err = bma2x2_parse_dt(&client->dev, pdata);
		if (err) {
			BMA2X2_ERR("%s line:%d Failed to parse device tree\n", __func__,__LINE__);
			err = -EINVAL;
	...
	if (!pdata) {
		BMA2X2_ERR("%s line:%d Cannot get device platform data\n", __func__,__LINE__);
		err = -EINVAL;
		goto kfree_exit;
	}
	data->pdata = pdata;
	...
	if (err) {
		BMA2X2_ERR("%s line:%d Failed to get sensor regulators\n", __func__,__LINE__);
		err = -EINVAL;
		goto free_i2c_clientdata_exit;
	}
	err = bma2x2_power_ctl(data, true);
	if (err) {
		BMA2X2_ERR("%s line:%d ailed to enable sensor power\n", __func__,__LINE__);
		err = -EINVAL;
	...
	if (bma2x2_soft_reset(client) < 0) {
		BMA2X2_ERR("%s line:%d i2c bus write error, pls check HW connection\n", __func__,__LINE__);
		err = -EINVAL;
	...
	if (pdata->int_en) {
		...
		if (err) {
			BMA2X2_ERR("%s line:%d Failed to init pinctrl err=%d\n", __func__,__LINE__,err);
			err = -EINVAL;
			goto disable_power_exit;
		}
		/* check interrupt feature enable state */
		if ((pdata->use_int2 && (!BMA2x2_IS_INT2_ENABLED())) ||
			...
			BMA2X2_ERR("%s line:%d Interrupt support is not enabled, int1=%d, int2=%d use_int2=%d\n", 
				__func__,__LINE__,BMA2x2_IS_INT1_ENABLED(),
		...
		if (err) {
			BMA2X2_ERR("%s line:%d Failed to get interrupt gpio, err=%d\n", __func__,__LINE__,err);
			err = -EINVAL;
		...
		BMA2X2_ERR("%s line:%d IRQ=%d, use_int2=%d, int_flag=0x%x\n", 
			__func__,__LINE__,data->IRQ, pdata->use_int2, data->int_flag);

		err = request_irq(data->IRQ, bma2x2_irq_handler,
			data->int_flag, "bma2x2", data);
		if (err) {
			BMA2X2_ERR("%s line:%d Could not request irq\n", __func__,__LINE__);
			goto free_interrupt_gpio;
		}
		disable_irq(data->IRQ);
		INIT_WORK(&data->irq_work, bma2x2_irq_work_func);
	} else {
		//INIT_DELAYED_WORK(&data->delay_work, bma2x2_work_func);
		hrtimer_init(&data->poll_timer, CLOCK_MONOTONIC,
					HRTIMER_MODE_REL);
		data->poll_timer.function = bma_timer_func;
		INIT_WORK(&data->delay_work.work, bma2x2_work_func);
	}

	data->data_wq = alloc_workqueue("bma2x2_data_work",
				WQ_UNBOUND | WQ_MEM_RECLAIM | WQ_HIGHPRI, 1);
	//create_freezable_workqueue("bma2x2_data_work");
	if (!data->data_wq) {
		BMA2X2_ERR("%s line:%d Cannot get create workqueue!\n", __func__,__LINE__);
		goto free_irq_exit;
	}
	...
	if (!dev) {
		BMA2X2_ERR("%s line:%d Cannot allocate input device\n", __func__,__LINE__);
		err = -ENOMEM;
	...
	if (!dev_interrupt) {
		BMA2X2_ERR("%s line:%d Cannot allocate input interrupt device\n", __func__,__LINE__);
		err = -ENOMEM;
		goto destroy_workqueue_exit;
	}
	...
	if (err < 0) {
		BMA2X2_ERR("%s line:%d Cannot register input device\n", __func__,__LINE__);
		goto free_irq_exit;
	}
	...
	if (err < 0) {
		BMA2X2_ERR("%s line:%d Cannot register input interrupt device\n", __func__,__LINE__);
		goto free_irq_exit;
	}
	...
	if (IS_ERR(data->g_sensor_class)) {
		...
		BMA2X2_ERR("%s line:%d could not allocate g_sensor_class\n", __func__,__LINE__);
		goto free_irq_exit;
	}
	...
	if (unlikely(IS_ERR(data->g_sensor_dev))) {
		...
		BMA2X2_ERR("%s line:%d could not allocate g_sensor_dev\n", __func__,__LINE__);
	...
	err = sysfs_create_group(&data->g_sensor_dev->kobj,
			&bma2x2_sig_motion_attribute_group);
	if (err < 0) {
		BMA2X2_ERR("%s line:%d could not create sysfs for sig motion sensor\n", __func__,__LINE__);
	...
	if (IS_ERR(data->g_sensor_class_doubletap)) {
		...
		BMA2X2_ERR("%s line:%d could not allocate g_sensor_class_doubletap\n", __func__,__LINE__);
	...
	if (unlikely(IS_ERR(data->g_sensor_dev_doubletap))) {
		...
		BMA2X2_ERR("%s line:%d could not allocate g_sensor_dev_doubletap\n", __func__,__LINE__);
	...
	err = sysfs_create_group(&data->g_sensor_dev_doubletap->kobj,
			&bma2x2_double_tap_attribute_group);
	if (err < 0) {
		BMA2X2_ERR("%s line:%d could not create sysfs for double tap sensor\n", __func__,__LINE__);
	...
	if (err < 0) {
		BMA2X2_ERR("%s line:%d Cannot create sysfs for bma2x2\n", __func__,__LINE__);
		goto remove_dtap_sysfs_exit;
	}

	dev_acc = bst_allocate_device();
	if (!dev_acc) {
		BMA2X2_ERR("%s line:%d Cannot allocate bst device\n", __func__,__LINE__);
		err = -ENOMEM;
	...
	if (err < 0) {
		BMA2X2_ERR("%s line:%d Cannot register bst device\n", __func__,__LINE__);
...
/* CQ_hw00003916 yanfei 20151121 end > */
	if (err < 0) {
		BMA2X2_ERR("%s line:%d Cannot create sysfs for bst_acc.\n", __func__,__LINE__);
		goto bst_free_exit;
	}
#ifdef CONFIG_HAS_EARLYSUSPEND
	...
	if (err) {
		BMA2X2_ERR("%s line:%d create class device file failed!\n", __func__,__LINE__);
				...
				struct bma2x2_data *data)
{
	...
	if (err < 0) {
		BMA2X2_ERR("%s line:%d get state bandwidth failed\n", __func__,__LINE__);
		return err;
	}
	err = bma2x2_get_range(client, &(data->range));
	if (err < 0) {
		BMA2X2_ERR("%s line:%d get state range failed\n", __func__,__LINE__);
		return err;
	}
	return err;
}
